...
1. Find the app key `TodoListScope` and replace the existing value with Scope if you changed the name from `user_impersonation`.
...
For details about the way the code to sign-in users was created, see [How was the code created](../1-WebApp-OIDC/1-1-MyOrg/README.md#How-was-the-code-created) section, of the README.md file located in the sibling folder named **1. Enable your Web Apps to sign-in users**.

In this section here we would only detail the  additional code added to:
...
If you went through the section [How to deploy this sample to Azure](../1-WebApp-OIDC/1-1-MyOrg/README.md#How-to-deploy-this-sample-to-Azure) in the first part of this tutorial, be careful **not to repeat** the steps in deploying the aspnetcore web client app.  Note that the client web application **will need redeployment** to account the changes to the appsettings.json file and more.
...
- Articles about the new Microsoft Identity Platform are at [http://aka.ms/aaddevv2](http://aka.ms/aaddevv2), with a focus on:
  ...
  - [The OAuth 2.0 protocol in Azure AD](https://docs.microsoft.com/en-us/azure/active-directory/develop/active-directory-protocols-oauth-code)

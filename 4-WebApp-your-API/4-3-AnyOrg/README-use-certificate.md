...
4. Run the [Configure.ps1](./AppCreationScripts-withCert/Configure.ps1) script to re-create the App Registration. The script will also create `.pfx` file(s) (e.g. WebApi_MultiTenant_v2.pfx) that you can upload to Key Vault later. When asked about a password, remember it - you will need the password when uploading the certificate.
...
> There are two projects needing certificates in this sample. Perform the steps below for each of them.
...
```PowerShell
$cert = New-SelfSignedCertificate -Subject "CN=WebApi_MultiTenant_v2" -CertStoreLocation "Cert:\CurrentUser\My" -KeyExportPolicy Exportable -KeySpec Signature -KeyLength 2048 -KeyAlgorithm RSA -HashAlgorithm SHA256
```

You can now export a public key (*.cer* file) and a public + private key combination (*.pfx* file) to use in your app:

```Powershell
Export-Certificate -Cert $cert -FilePath "C:\Users\diego\Desktop\WebApi_MultiTenant_v2.cer" ## Specify your preferred location

$mypwd = ConvertTo-SecureString -String "{myPassword}" -Force -AsPlainText  ## Replace {myPassword}
Export-PfxCertificate -Cert $cert -FilePath "C:\Users\diego\Desktop\WebApi_MultiTenant_v2.pfx" -Password $mypwd ## Specify your preferred location
```
...
```bash
openssl req -x509 -newkey rsa:2048 -keyout WebApi_MultiTenant_v2.key -out WebApi_MultiTenant_v2.cer -subj "/CN=WebApi_MultiTenant_v2" -nodes

Generating a RSA private key
.........................................................
writing new private key to 'WebApi_MultiTenant_v2.key'
```

The following files should be generated: *WebApi_MultiTenant_v2.key*, *WebApi_MultiTenant_v2.cer*

You can generate a WebApi_MultiTenant_v2.pfx (certificate + private key combination) with the command below:

```bash
openssl pkcs12 -export -out CertificateName.pfx -inkey WebApi_MultiTenant_v2.key -in WebApi_MultiTenant_v2.cer
```

Enter an export password when prompted and make a note of it. The following file should be generated: *WebApi_MultiTenant_v2.pfx*.
...
1. Click on **Upload** certificate and select the certificate file to upload (e.g. *WebApi_MultiTenant_v2*).
...
> Perform the steps below for the service app (WebApi_MultiTenant_v2)
...
```json
    "ClientCertificates": [
        {
            "SourceType": "Path",
            "CertificateDiskPath": "<path to certificate e.g. c:\Users\diego\Desktop\WebApi_MultiTenant_v2.pfx",
...
> Perform the steps below for the client app (WebApp_MultiTenant_v2)
...
> There are two projects that can make use of managed identity in this sample. Perform the steps below for each of them.

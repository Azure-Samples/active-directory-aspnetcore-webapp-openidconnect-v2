...
#> 
   ...
   Write-Host "Creating the AAD application (WebApp-SqlCache-V2)"
   # Get a 2 years application key for the webApp Application
   $pw = ComputePassword
   $fromDate = [DateTime]::Now;
   $key = CreateAppKey -fromDate $fromDate -durationInYears 2 -pw $pw
   $webAppAppKey = $pw
   $webAppAadApplication = New-AzureADApplication -DisplayName "WebApp-SqlCache-V2" `
                                                  -HomePage "https://localhost:44394/" `
                                                  -LogoutUrl "https://localhost:44394/signout-oidc" `
                                                  -ReplyUrls "https://localhost:44394/", "https://localhost:44394/signin-oidc" `
                                                  -IdentifierUris "https://$tenantName/WebApp-SqlCache-V2" `
   ...
   Write-Host "Done creating the webApp application (WebApp-SqlCache-V2)"
   ...
   Add-Content -Value "<tr><td>webApp</td><td>$currentAppId</td><td><a href='$webAppPortalUrl'>WebApp-SqlCache-V2</a></td></tr>" -Path createdApps.html
   ...
   $configFile = $pwd.Path + "\..\WebApp-Cache-V2\appsettings.json"

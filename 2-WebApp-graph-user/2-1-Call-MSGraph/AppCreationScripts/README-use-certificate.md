...
3. Run the [Configure-withCertCertificates.ps1](./Configure-withCertCertificates.ps1) script to re-create the App Registration. The script will also create `.pfx` file(s) (e.g. WebApp-OpenIDConnect-DotNet-graph-v2.pfx) that you can upload to Key Vault later. When asked about a password, do remember it - you will need the password when uploading the certificate.
...
```PowerShell
$cert = New-SelfSignedCertificate -Subject "CN=WebApp-OpenIDConnect-DotNet-graph-v2" -CertStoreLocation "Cert:\CurrentUser\My" -KeyExportPolicy Exportable -KeySpec Signature -KeyLength 2048 -KeyAlgorithm RSA -HashAlgorithm SHA256
```

You can now export a public key (*.cer* file) and a public + private key combination (*.pfx* file) to use in your app:

```Powershell
Export-Certificate -Cert $cert -FilePath "C:\Users\diego\Desktop\WebApp-OpenIDConnect-DotNet-graph-v2.cer" ## Specify your preferred location

$mypwd = ConvertTo-SecureString -String "{myPassword}" -Force -AsPlainText  ## Replace {myPassword}
Export-PfxCertificate -Cert $cert -FilePath "C:\Users\diego\Desktop\WebApp-OpenIDConnect-DotNet-graph-v2.pfx" -Password $mypwd ## Specify your preferred location
```
...
```bash
openssl req -x509 -newkey rsa:2048 -keyout WebApp-OpenIDConnect-DotNet-graph-v2.key -out WebApp-OpenIDConnect-DotNet-graph-v2.cer -subj "/CN=WebApp-OpenIDConnect-DotNet-graph-v2" -nodes

Generating a RSA private key
.........................................................
writing new private key to 'WebApp-OpenIDConnect-DotNet-graph-v2.key'
```

The following files should be generated: *WebApp-OpenIDConnect-DotNet-graph-v2.key*, *WebApp-OpenIDConnect-DotNet-graph-v2.cer*

If you need, you can generate a WebApp-OpenIDConnect-DotNet-graph-v2.pfx (certificate + private key combination) with the command below:

```bash
openssl pkcs12 -export -out CertificateName.pfx -inkey WebApp-OpenIDConnect-DotNet-graph-v2.key -in WebApp-OpenIDConnect-DotNet-graph-v2.cer
```

Enter an export password when prompted and make a note of it. The following file should be generated: *WebApp-OpenIDConnect-DotNet-graph-v2.pfx*.
...
1. Click on **Upload** certificate and select the certificate file to upload (e.g. *WebApp-OpenIDConnect-DotNet-graph-v2*).
...
> Perform the steps below for the webApp app (WebApp-OpenIDConnect-DotNet-graph-v2)
...
```json
    "ClientCertificates": [
        {
            "SourceType": "Path",
            "CertificateDiskPath": "C:\GitHub\active-directory-aspnetcore-webapp-openidconnect-v2\2-WebApp-graph-user\2-1-Call-MSGraph\AppCreationScripts\WebApp-OpenIDConnect-DotNet-graph-v2.pfx",
            "CertificateThumbprint": "D96030FE18109B03B70F0142C27E934D13009B17"
        }
    ]
```

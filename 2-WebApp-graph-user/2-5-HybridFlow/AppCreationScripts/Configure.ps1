...
Function ConfigureApplications {
    ...
    if ($tenantId -eq "") {
        Connect-MgGraph -Scopes "User.Read Mail.Read" -Environment $azureEnvironmentName
        $tenantId = (Get-MgContext).TenantId
    }
    else {
        Connect-MgGraph -TenantId $tenantId -Scopes "User.Read Mail.Read" -Environment $azureEnvironmentName
    }
    ...
    $downstreamApiSettings = [ordered]@{ "BaseUrl" = "https://graph.microsoft.com/v1.0"; "Scopes" = "user.read mail.read"; };
